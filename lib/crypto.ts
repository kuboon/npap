...
*/
async function aesDecrypt(key: CryptoKey, iv: Uint8Array, encrypted: ArrayBuffer) {
...
export async function encAndWrap(wrapperKey: CryptoKey, plain: ArrayBuffer): Promise<EncData>{
  ...
  const key = await subtle.wrapKey(
    'raw', aeskey, wrapperKey, 'RSA-OAEP'
  )
...
export async function unwrapAndDec(unwrapperKey: CryptoKey, data: EncData){
  ...
  const aeskey = await subtle.unwrapKey(
    'raw', key, unwrapperKey, 'RSA-OAEP', 'AES-CBC', false, ['decrypt']
  )
